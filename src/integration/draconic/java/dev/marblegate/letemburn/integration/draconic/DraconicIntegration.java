...
public final class DraconicIntegration {
    public DraconicIntegration() {
        if (!ModIntegration.DRACONIC_EVOLUTION.loaded()) {
            return;
        }
        ImpactPayloadRegistry.INSTANCE.register(DraconicReactorImpactAdapter.INSTANCE);
        LetEmBurn.LOGGER.info("Initialized optional compatibility for {}", ModIntegration.DRACONIC_EVOLUTION.modId());
    }
}
